...
def _normalize_libdir(path):
    ...
    """
    for triple in ARCH_CPU.values():
        debian_libdir = "lib/%s-linux-gnu/" % triple
...
def _get_installed_paths(introspect_json):
    ...
    res = {
        _relativize_build_path(build_prefixes, path_build): paths.relativize(path_installed, prefix)
        for path_build, path_installed in installed_paths_.items()
    }

    for triple in ARCH_CPU.values():
        res = {k.format(arch = triple): v.format(arch = triple) for k, v in res.items()}
    ...
    if contrib_name in CONTRIB_INSTALLED_PATHS_OVERRIDE:
        ...
        for cspec in overrides:
            ...
            if is_compatible:
                ...
                for triple in ARCH_CPU.values():
                    contrib_paths = [p.format(arch = triple) for p in contrib_paths]
                break
