...
| <a id="pg_build-deps_buildtime"></a>deps_buildtime |  Optional list of dependency tarballs from rules_distroless packages. These will be combined into a sysroot and made available to the meson build via environment variables (PKG_CONFIG_SYSROOT_DIR, CFLAGS, LDFLAGS, etc.).   |  `None` |
