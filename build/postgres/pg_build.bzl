...
def _meson_common_args(pg_src, build_options, auto_features, sysroot_tarball = None):
    ...
    if sysroot_tarball:
        build_data.append(sysroot_tarball)
    ...
    if sysroot_tarball:
        ...
        env_sysroot = dict(
            ...
            SYSROOT_DIR = "$$(mkdir -p $$EXT_BUILD_ROOT/sysroot && tar -xf $(execpath {sysroot}) -C $$EXT_BUILD_ROOT/sysroot && echo $$EXT_BUILD_ROOT/sysroot)".format(
    ...
    # tools from the sysroot (like llvm-config for JIT, msgfmt for i18n) are available.
    # The system LLVM path (/usr/lib/llvm-19/bin) must come before the sysroot
    # path so that meson finds the system clang (which is in Docker) rather
    # than looking for clang in the sysroot (where only llvm-config exists).
    ...
    if sysroot_tarball:
        ...
        path_components.append("$$SYSROOT_DIR/usr/lib/llvm-19/bin")  # llvm-config
...
def _pg_build_introspect(name, pg_src, build_options, auto_features, sysroot_tarball = None):
    meson_common_args = _meson_common_args(
        pg_src = pg_src,
        build_options = build_options,
        auto_features = auto_features,
...
def pg_build(name, pg_src, build_options, auto_features, buildtime_dependencies = None, pg_version = None):
    """
    ...
    Args:
        ...
        buildtime_dependencies (list[str]): Optional list of dependency tarballs
            from rules_distroless packages. These will be extracted into a
    ...
    if buildtime_dependencies:
        sysroot_name = "{}--sysroot".format(name)
        pg_sysroot(
            name = sysroot_name,
            dependencies = buildtime_dependencies,
        )
        sysroot_tarball = ":{}".format(sysroot_name)

    _pg_build_meson(name, pg_src, build_options, auto_features, sysroot_tarball)
    _pg_build_introspect(name, pg_src, build_options, auto_features, sysroot_tarball)
